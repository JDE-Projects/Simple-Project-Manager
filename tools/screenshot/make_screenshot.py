#!/usr/bin/env python3
"""Regenerate screenshots/project_manager-light-dark.png.

Simple Project Manager is a desktop app: at screenshot time there is no web
...
its assets from a temp folder, seeds a sample project straight into STATE, and
drives the page's own render functions to produce the picture.
...
OUT_IMAGE = os.path.join(REPO_ROOT, "screenshots",
                         "project_manager-light-dark.png")

# Each theme is laid out at this size and captured at half scale, giving two
# 900x560 halves and the 1800x560 composite the README uses. The height is
# tuned to close just under the last panel, before the version bar, with no
# empty band beneath the content.
LAYOUT_WIDTH = 1800
LAYOUT_HEIGHT = 1120
CAPTURE_SCALE = 0.5
...
def read_app_version() -> str:
    path = os.path.join(REPO_ROOT, "simple_project_manager.py")
...
def stage_ui(temp_dir: str) -> None:
    ...
    shutil.copy2(os.path.join(REPO_ROOT, "simple_project_manager-UI.html"),
                 os.path.join(temp_dir, "index.html"))
    shutil.copy2(os.path.join(REPO_ROOT, "simple_project_manager.png"),
                 temp_dir)
...
def build_setup_script(version: str) -> str:
    """JavaScript that seeds STATE with the sample project and drives the
    page's own render path.

    The UI's boot() only runs on the pywebviewready event, which never fires
    in a plain browser, so this calls the same functions boot() would: it
    sets the version bar text, then updateMeta(), initFilterBar(),
    autoCollapseDone(), and render(), each guarded so the script still works
    ...
    return (
        ...
        "if (typeof initFilterBar === 'function') initFilterBar();"
        "if (typeof autoCollapseDone === 'function') autoCollapseDone();"
...
def main(argv: list) -> None:
    ...
    temp_dir = tempfile.mkdtemp(prefix="spm-screenshot-")
    httpd = None
